/* 
 * JSON Data will be generated after every 10s, but send only when it is different that it`s previous one. 
...
void loop() {
  ...
  if ( millis() >= timer ) { timer = millis()+10000UL; json_creater(); }       /* Send data after every 10s if line is up ( running ) */
  
  if ( DT_Counter == 12 ) { json_sender(); }                                   /* Send data after every 2 min if line is down ( not running ) */
