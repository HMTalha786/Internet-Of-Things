...
void Read_EA_Param() {
  ...
  Serial.print("Total 3 Phase True Energy "); 
  TTP = TTE1 + TTE2 + TTE3;       
  Serial.println(TTP);   
  delay(50);
  ...
  if ( sq( AP_L1 + AP_L2 + AP_L3 ) > sq( TP_L1 + TP_L2 + TP_L3 ) ) { TRP = sqrt( sq( AP_L1 + AP_L2 + AP_L3 ) - sq( TP_L1 + TP_L2 + TP_L3 ) ); } else { TRP = 0.0; }
  Serial.println(TRP);  
  delay(50); 
